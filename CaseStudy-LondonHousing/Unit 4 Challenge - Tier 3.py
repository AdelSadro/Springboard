...
properties_decade_mean = properties.resample('10A').mean()


# In[14]:


properties_decade_mean
...
properties_decade_mean.plot(kind='line', figsize=(24, 12))
plt.legend(bbox_to_anchor=(1.15, 1), loc='upper right')
plt.title('properties average value in decades')
plt.ylabel('Price')
plt.savefig('properties_average_value_in_decades.png', bbox_inches='tight')


# In[38]:


properties_decade_mean_melt = pd.melt(properties_decade_mean, var_name='Borough', value_name='Average Price')


# In[39]:


properties_decade_mean_melt.sort_values(by=['Average Price'], ascending=False)


# In[46]:


dfi.export(properties_decade_mean_melt.sort_values(by=['Average Price'], ascending=False), 'sorted.png', max_rows=10)
...
properties['Year'] = pd.DatetimeIndex(properties.index).year
properties['Month'] = pd.DatetimeIndex(properties.index).month
...
def create_price_ratio(df, year_tup):
    ...
    london_boroughs = ['City of Westminster', 'Kensington & Chelsea', 'Hammersmith & Fulham', 'Wandsworth', 'Lambeth'
                    ...
                    ,'Richmond upon Thames', 'Kingston upon Thames', 'Merton', 'Sutton', 'Croydon', 'Bromley'
                    ,'Lewisham', 'Greenwich', 'Bexley', 'Havering', 'Barking & Dagenham', 'Redbridge', 'Newham'
                    ,'Waltham Forest', 'Haringey', 'Enfield', 'Barnet', 'Harrow', 'Hillingdon', 'Year']
    
    for col in df.columns:
        if col not in london_boroughs:
            del_col.append(col)
    ...
    for tup in year_tup:
        if tup not in range(1995, 2026):
            ...
            df_temp = df.loc[df['Year'] == tup, :].mean()
            df_mean = df_mean.append(df_temp, ignore_index=True)

    df_mean.loc[2] = df_mean.loc[0]/df_mean.loc[1]
    df_ratio = df_mean.drop(['Year'], axis=1)
    return(df_ratio.loc[2])
    

ratio = create_price_ratio(properties, (1998, 2018))
ratio
...
dfi.export(pd.DataFrame(ratio), 'ratio.png')
